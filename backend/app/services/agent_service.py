...
3. **生成图片序列**：基于分镜脚本，为每个镜头生成对应的首帧图片
   - 🆕 **角色一致性规则（重要 · 多角色场景必须遵守）**：
     * 识别分镜中所有具名角色（如"张伟"、"李娜"等）
     * 角色【首次出场】→ 使用 generate_volcano_image，并明确记住该角色名 → 图片路径的对应（在内心建立角色表）
     * 角色【再次出场】（前面镜头已生成过他的首图）→ **必须**使用 edit_volcano_image，传入该角色首图路径作为 image_url，以保持人物形象一致；**禁止**对同一角色重复使用 generate
     * 多个角色【同框】→ 使用 edit_volcano_image，image_url 传【主角】（在该镜头首次提到的角色）参考图，prompt 中补充其他角色的外貌描述（如"另有李娜：长卷发红毛衣"）
     * 镜头是【空镜】（无角色出场）→ 使用 generate_volcano_image，无需参考图
   - 每个图片的提示词应基于对应镜头的场景描述
