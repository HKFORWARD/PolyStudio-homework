# 第 4 周作业 · 解答

> 关联文档：[作业要求](作业要求.md)
> 关联代码：`~/development/AIGC/learning/AI_Video/week-04/PolyStudio/`
> GitHub 提交仓库：（待填）

---

## Task 1 · 当前长视频 Agent 基础能力分析

> **代码依据**
> Prompt：[`agent_service.py`](backend/app/services/agent_service.py) L75-77、L82-134
> 工具：[`volcano_image_generation.py`](backend/app/tools/volcano_image_generation.py)、[`volcano_video_generation.py`](backend/app/tools/volcano_video_generation.py)、[`video_concatenation.py`](backend/app/tools/video_concatenation.py)

### 1.1 现有工作流

**六步链路**（Prompt L82-134 规定）：

| # | 阶段 | 类型 | 工具 |
|---|------|------|------|
| ① | 需求分析 | 纯 Prompt | — |
| ② | 分镜生成 | 纯 Prompt | — |
| ③ | 生成图片序列 | 工具调用 | `generate_volcano_image` |
| ④ | 生成视频片段 | 工具调用 | `generate_volcano_video`（mode=`image`）|
| ⑤ | 拼接成片 | 工具调用 | `concatenate_videos` |
| ⑥ | 质量检查 | 纯 Prompt | — |

**数据传递（混合态）**

- **结构化层**：工具调用的入参与返回是 JSON，按 Prompt L75-77 自动落入 `messages` 对话历史
- **非结构化层**：分镜本身是 LLM 输出的**自然语言段落**——下游每一步生图/生视频时，LLM 必须**重新解析自己上一轮输出**，手工提取"镜头序号 / 描述 / 时长"，再填到工具参数里

> 💡 **结论**：短板在**分镜层无结构化中间态**，不在工具调用层。

---

### 1.2 现有角色一致性

| 子问题 | 现状 |
|--------|------|
| **如何保证多镜头一致？** | 长视频流程未规定，仅靠每镜头 `generate` 时 prompt 文字一致 |
| **依赖什么输入？** | Prompt 未要求任何角色参考图作为必备输入 |
| **edit vs generate？** | Prompt L107-111 明文规定**用 `generate`**；`edit_volcano_image` 虽已注册（L30）但长视频流程未引用 |
| **Prompt 中的规则？** | 全文 **0 处**出现"角色 / 主角 / 人物"；唯一相关约束是"风格一致（相同主题、相似色调、统一尺寸）" |

> ⚠️ **结论**：**长视频流程下，角色一致性无任何显式机制**。作业题干说的"基于同一张角色图做 `edit`"是用户在普通对话里的外挂玩法，**不是 Prompt 工作流的一部分**。

---

### 1.3 现有时长处理

| 子问题 | 现状 |
|--------|------|
| **分镜中如何体现时长？** | 文本格式 `镜头N（X秒）：[描述]`（Prompt L94-104 示例） |
| **单段时长谁决定？** | **LLM 读分镜文本后手动填** `duration` 参数；默认 5 秒，范围 4-12 秒（[L479](backend/app/tools/volcano_video_generation.py#L479)）|
| **总时长校验？** | ❌ 仅 Prompt L116 一句"确保总和不超过总时长" → **靠 LLM 心算** |
| **接口返回 ≠ 请求 怎么处理？** | ❌ **完全不处理** |

**两条铁证（拼接工具的"时长黑洞"）**

| 文件 | 现象 |
|------|------|
| [`video_concatenation.py` L217](backend/app/tools/video_concatenation.py#L217) | 内部拿到 `final_clip.duration`，**只写日志** |
| [`video_concatenation.py` L300-306](backend/app/tools/video_concatenation.py#L300-L306) | 返回 JSON 字段：`video_url / video_count / output_filename / message` —— **无 duration** |
| [`volcano_video_generation.py` L572](backend/app/tools/volcano_video_generation.py#L572) | 返回的 `duration` 是**请求值原样回填**，非 API 实际返回值 |

> 🔴 **结论**：LLM 全程拿不到任何"实际时长"信号，"接口返回时长 ≠ 请求"的情况完全不处理。

---

### 1.4 数据流图

![PolyStudio 长视频生成数据流图](assets/task1_dataflow.png)

<details>
<summary>📐 查看 SVG 源码 / Mermaid 源码（可编辑）</summary>

**SVG 源文件**：[`assets/task1_dataflow.svg`](assets/task1_dataflow.svg)

**Mermaid 等价描述**：

```mermaid
flowchart LR
    U([👤 用户输入]) --> A[① 需求分析]
    A --> B[② 分镜生成<br/>📝 自然语言文本]
    B -.LLM 自解析<br/>镜头/时长.-> C[③ generate_volcano_image<br/>🎨 每镜头独立生图]
    C -.image_url<br/>经对话历史.-> D[④ generate_volcano_video<br/>🎬 mode=image<br/>duration 手填]
    D -.video_urls 列表.-> E[⑤ concatenate_videos<br/>🔗 拼接<br/>⚠️ 不返回实际时长]
    E --> F[⑥ 质量检查]
    F --> O([🎞️ 成片])

    style B fill:#fff3cd,stroke:#856404,color:#000
    style E fill:#ffd6d6,stroke:#a33,color:#000
    style A fill:#f0f0f0,stroke:#999,color:#000
    style F fill:#f0f0f0,stroke:#999,color:#000
    style C fill:#d4edda,stroke:#28a745,color:#000
    style D fill:#d4edda,stroke:#28a745,color:#000
```

</details>


---

## Task 2 · 多角色一致性扩展方案

> **设计起点**：Task 1 实证当前长视频流程是 **σ 纯文生图**（log 实测：[`polystudio_20260426.log`](backend/logs/polystudio_20260426.log) L463-575 显示 6 镜头全部 `generate_volcano_image`，0 次 `edit`），一致性仅靠 prompt 文字。本方案目标：**把 LLM 从 σ 拉到 β（单角色像素级一致）+ 预留 α 升级位（多角色同框）**。

### 2.1 数据结构设计

在已有的 `Canvas` 模型（[`history_service.py:11`](backend/app/services/history_service.py#L11)）上**新增 `characters` 字段**，绑在项目级——同一项目内的角色不污染其它项目。

```python
class Character(BaseModel):
    id: str            # "char_001"
    name: str          # "小明"  ← LLM 与用户用此名交流
    image_url: str     # "/storage/images/xxx.jpg"  ← 参考图路径
    description: str   # "20岁男生，黑色短发，蓝色卫衣"  ← B 类角色文本兜底用

class Canvas(BaseModel):
    id: str
    name: str
    # ... 已有字段 ...
    characters: List[Character] = []   # ← 新增
```

**登记方式**：通过新增工具 `register_character(name, description, image_url=None)` 显式登记；可选支持外部带入（用户上传一张图 + 一句话描述 → 自动登记）。

---

### 2.2 分镜描述扩展

在原分镜模板（[`agent_service.py` L94-104](backend/app/services/agent_service.py#L94-L104)）基础上**新增 `[出场]` 标签**，一行内可读、对 Prompt 友好：

**改造前**

```
镜头1（5秒）：[详细场景描述]
```

**改造后**

```
角色表：
- 小明：20岁男生，黑色短发，蓝色卫衣
- 小红：22岁女生，长发，白色连衣裙

镜头1（5秒）[出场: 小明]：小明在咖啡馆窗边，捧着热咖啡看书
镜头2（5秒）[出场: 小红]：小红推门走进咖啡馆，手中拿着雨伞
镜头3（5秒）[出场: 小明, 小红]：小明抬头注意到小红，两人目光相遇
镜头4（5秒）[出场: 无]：咖啡馆窗外的雨景空镜
```

`[出场: ...]` 是机器友好的强约束，LLM 只需正则解析或字符串切片即可还原"本镜头出场角色"。

---

### 2.3 Agent 选图与生图逻辑（伪代码）

```python
def render_shot_image(canvas: Canvas, shot: Shot) -> str:
    """为一个镜头选择参考图并调用合适的工具"""
    actors = shot.parse_actors()  # 解析 [出场: A, B]
    
    # ─ 情况 0：空镜（无角色出场）───────────────
    if not actors:
        return generate_volcano_image(prompt=shot.description)
    
    # ─ 情况 1：单角色 ─────────────────────────
    if len(actors) == 1:
        char = canvas.get_character(actors[0])
        if char and char.image_url:
            # 已登记 → 用参考图做 edit（β：主角锚定）
            return edit_volcano_image(
                image_url=char.image_url,
                prompt=shot.description
            )
        else:
            # 首次出场 → generate + 自动登记
            full_prompt = f"{shot.description}; {char.description}" if char else shot.description
            new_img = generate_volcano_image(prompt=full_prompt)
            if not char:
                register_character(name=actors[0], image_url=new_img, description=...)
            else:
                update_character_image(actors[0], new_img)
            return new_img
    
    # ─ 情况 2：多角色同框 ─────────────────────
    # 默认走 β（兜底）：主角图 + 文本描述其他角色
    primary = actors[0]
    primary_ref = canvas.get_character(primary).image_url
    secondary_desc = "; ".join([
        f"另有{a}：{canvas.get_character(a).description}"
        for a in actors[1:]
    ])
    return edit_volcano_image(
        image_url=primary_ref,
        prompt=f"{shot.description}; {secondary_desc}"
    )
    
    # ─ α 升级方向（待 Schema 改造 + API 实测）─
    # refs = [canvas.get_character(a).image_url for a in actors]
    # return edit_volcano_image(image_urls=refs, prompt=shot.description)
```

**核心判断三叉**：

| 出场角色数 | 工具 | 参考图 | Prompt |
|---|---|---|---|
| 0（空镜）| `generate` | — | 镜头描述 |
| 1 | `edit` | 该角色参考图 | 镜头描述 |
| ≥2（β 默认）| `edit` | **主角**参考图 | 描述 + 其他角色文本兜底 |
| ≥2（α 升级）| `edit` | **多图数组** | 描述 |

---

### 2.4 Prompt 改动点

需要在 [`agent_service.py`](backend/app/services/agent_service.py) 长视频工作流 Prompt（L82-134）插入/修改 4 处：

| # | 位置 | 改动 |
|---|------|------|
| ① | **新增节** `<角色管理>` | 解释 `register_character` / `list_characters` 工具用途、登记时机 |
| ② | **步骤 2 分镜生成**（L87-105）| 模板加入"角色表 + `[出场: ...]` 标注"格式约束 |
| ③ | **步骤 3 生成图片序列**（L107-111）| 改为 "先解析 `[出场: ...]` → 按三叉规则选 `generate` / `edit`" |
| ④ | **新增条** "**首次出场必登记**" | 任何角色首次 generate 后立即调 `register_character` 落库 |

`agent_service.py:30` 工具列表追加 `register_character_tool` 与 `list_characters_tool`。

---

### 2.5 架构图

![Task 2 多角色一致性架构](assets/task2_architecture.png)

<details>
<summary>📐 查看 SVG 源码</summary>

**SVG 源文件**：[`assets/task2_architecture.svg`](assets/task2_architecture.svg)

</details>

---

### 2.6 落地路径建议

**最小可行实现**（≈1 天工作量）：

1. `Canvas` 模型加 `characters` 字段，`HistoryService` 加 `add_character / get_character / update_character_image` 方法
2. 新增 `register_character_tool` / `list_characters_tool`，在 `agent_service.py:26-34` 注册
3. 改写长视频工作流 Prompt（4 处改动）
4. 实测一组多角色长视频，对比 β 与 σ 的一致性

**进阶 α 升级**（额外 ≈0.5 天）：

5. `EditVolcanoImageInput` 改为 `image_url: Union[str, List[str]]`
6. `prepare_image_input` 支持列表批量处理
7. 实测火山 API 是否真支持图片数组（注释里说"或数组"但需验证）

---

## 自查清单

- [x] Task 1 分析覆盖三个子问题（工作流 / 角色一致性 / 时长处理）
- [x] Task 1 配数据流图（[`assets/task1_dataflow.png`](assets/task1_dataflow.png)）
- [x] Task 2 交付"方案与架构 + 伪代码"组合（满足"任选一种或组合"要求）
- [x] Task 2 配架构图（[`assets/task2_architecture.png`](assets/task2_architecture.png)）
- [ ] 推送到 GitHub 并填入提交仓库链接
- [ ] 在「作业提交入口」提交 GitHub 链接
