...
- 设计：§2.1-2.5（数据结构 + 标签 + 三叉决策 + 架构图）
- 实施：改 6 行 Prompt（[`agent_service.py`](backend/app/services/agent_service.py) L107-117）
- 实测：σ 基线（§A.1-A.5）+ β 对照（§A.6）

#### ⏳ 后续可做

- 完整落地：写 `Canvas.characters` 字段 + `register_character_tool` / `list_characters_tool`
- α 升级：改 `EditVolcanoImageInput` Schema 接受图片数组 + 实测火山 API
- 结构化解析：写 `[出场: A, B]` 标签 parser 替代 LLM 隐式解析

#### 📊 σ / β / α 对比

| 维度 | σ（现状）| β（本次完成）| α（待升级）|
|---|---|---|---|
| 是否用 edit | ❌ | ✅ 单图 | ✅ 多图数组 |
| 角色一致性 | 仅文字 | 主角像素锚定 | 全角色像素锚定 |
| 改动量 | — | 6 行 Prompt | + 1 个 Schema 字段 |
| 多角色配角效果 | 漂 | 较稳 | 完全稳 |
| 本次是否完成 | ✅ 实测 | ✅ 实测 | ⚠️ 仅设计 |
